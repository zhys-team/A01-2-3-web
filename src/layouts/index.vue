...
<script>
  ...
  import { queryOrgsByUserId } from '@/api/invoice.js'
  export default {
    name: 'Layout',
    data() {
      return { oldLayout: '' }
    ...
    created() {
      this.queryOrgsByUserId(this.$store.state.user.user.id)
    },
    methods: {
      ...mapActions({
      ...
      async queryOrgsByUserId(id) {
        let { data: result } = await queryOrgsByUserId(id)
        console.log(result)
        if (result.success) {
          let items = result.data
          let item = JSON.parse(JSON.stringify(result.data[0]))
          item.orgId = null
          item.orgMachine = null
          item.orgName = null
          item.orgTaxcode = null
          items.push(item)
          // items.forEach((eve) => {
          //   eve.speed = 0
          // })
          this.$store.commit('setOrgs', items)
          this.$store.commit('setOrg', this.$store.state.orgs[0])
        } else {
